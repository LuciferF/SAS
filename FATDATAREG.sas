...
data BodyFat;
    input Var_Num PctBodyFat1 PctBodyFat Density Age Weight Height
        Adioposity FatFreeWt Neck Chest Abdomen Hip Thigh
        Knee Ankle Biceps Forearm Wrist;
    datalines;
...
run;
proc sql;
CREATE TABLE Pearson_CORR AS
SELECT * FROM ( 
select best1 as Name, R1 as PearsonCorr, P1 as P_value from P1
UNION
select best2 as Name, R2 as PearsonCorr, P2 as P_value from P1
UNION
select best3 as Name, R3 as PearsonCorr, P3 as P_value from P1
UNION
select best4 as Name, R4 as PearsonCorr, P4 as P_value from P1
UNION
select best5 as Name, R5 as PearsonCorr, P5 as P_value from P1
UNION
select best1 as Name, R1 as PearsonCorr, P1 as P_value from P2
UNION
select best2 as Name, R1 as PearsonCorr, P2 as P_value from P2
UNION
select best3 as Name, R3 as PearsonCorr, P3 as P_value from P2
UNION
select best4 as Name, R4 as PearsonCorr, P4 as P_value from P2
UNION
select best5 as Name, R5 as PearsonCorr, P5 as P_value from P2
UNION
select best6 as Name, R6 as PearsonCorr, P6 as P_value from P2
UNION
select best7 as Name, R7 as PearsonCorr, P7 as P_value from P2
UNION
select best8 as Name, R8 as PearsonCorr, P8 as Pvalue from P2
UNION
select best9 as Name, R9 as PearsonCorr, P9 as P_value from P2
UNION
select best10 as Name, R10 as PearsonCorr, P10 as P_value from P2)
WHERE PearsonCorr > 0.3 and P_value < 0.0005;
run; 
proc sql NOPRINT; 
SELECT Name
INTO: INPUTS SEPARATED BY ' ' 
FROM Pearson_Corr;
run;
%PUT &INPUTS; 
proc reg data=bodyfat outest=fat_all_1 tableout plots(only)=(cp) alpha=.05 ;
fw: model pctbodyfat = Abdomen Adioposity Biceps Chest Forearm Hip Knee Neck Thigh Weight Wrist / selection=f;
title 'First Body Fat Model';
run;
proc contents
     data = fat_all_1
          noprint
...
proc sql NOPRINT;
select name
INTO: MODEL1 SEPARATED BY ' '
FROM (SELECT Name from data_info
WHERE VARNUM > 4);
%PUT &MODEL1;
proc reg data=bodyfat outest=fat_all_2 tableout plots(only)=(cp) alpha=.05 ;
bw: model pctbodyfat = Abdomen Adioposity Biceps Chest Forearm Hip Knee Neck Thigh Weight Wrist / selection=backward;
title 'Second Body Fat Model';
run;
proc contents
     data = fat_all_2
          noprint
          out = data_info2
                (keep = name varnum);
run;    
proc reg data=bodyfat outest=f_final tableout;
fw_final: model pctbodyfat =  Abdomen Weight Wrist Forearm;
title 'Final Body Fat Model';
run; 
quit;
